	...
	public static void main(String[] args) {
        ...
        do {
            ...
            try {
                ...
                final int execute = sc.nextInt();//入力された数字が何かによってエラーの表示を変えていく↓

                // 早期処理戻し
                if (execute == -1) break;
                // オペレーションエラー
                if (execute != 1) { //入力された数字が１と異なる場合”入力値が不正です”を表示
                    System.out.println(CONST_COMMON_MSG_ERROR_EXCEPTION);
                    continue;//入力された数値が異なる場合はこのエラー表示を繰り返す
                }

                System.out.println(CONST_COMMON_TASK_INPUT_NAME);
                parameter = sc.nextInt();

                switch (parameter) {//入力された数字が１の場合ヌルポを発生させる
				...
				case CONST_EXCEPTION_TRIGER_ARRAY_OUT_OF_BOUNDS://入力された数字が２の場合
